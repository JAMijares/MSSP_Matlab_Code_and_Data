...
%
% Syntax:  Main_RunToFailure_P2
%
% Outputs:
%    IndicatorsComparison_P2.fig
%    IndicatorsComparison_P2.png
%
...
pauseText={sprintf('Pause to increase\n interference to 10 mils \n + 1 min running to warm up'), ...
           sprintf('Continue operation'), ...
           sprintf('Pause to increase\n interference to 15 mils \n + 1 min running to warm up'), ...
           sprintf('Continue operation')};
for k=4:length(files)-2
    bar_xlim = ticks(2*k:2*k+1);
    bar_xlim(2) = bar_xlim(2)-0.5;
    bar_xlim(1) = bar_xlim(1)+0.5;
    [X,Y] = hatch_coordinates( bar_xlim  , yl , 1 , 1 ) ; %// this return coordinates to plot a hatch pattern
    plot(X,Y,'Color',[.6 .6 .6],'Marker','none','linewidth',1,'LineStyle',':'); hold on
    ...
    h=text(0.5*(bar_xlim(2)+bar_xlim(1)),yl(2)/2,pauseText{k-3},...
...
%% Add Legends
 legend([p{1}(1) p1(1) p2(1) p3(1)],'RMS','Kurtosis','CF','Enhanced Kurtosis','Orientation','Horizontal',...
          'Location','northoutside')
%%
imageName = 'IndicatorsComparison_P2';
